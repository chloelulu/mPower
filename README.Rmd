...
reactable(res1$power, resizable = T)
```
...
```{r echo=F, error=TRUE, message=FALSE, warning=FALSE, include=T, results="asis"}
reactable(res2$pOCR, resizable = T)
```
##### 2.2.2 Output2: Taxa-level power table ("aTPR" column indicates the average true positive rate)
```{r echo=F, error=TRUE, message=FALSE, warning=FALSE, include=T, results="asis"}
reactable(res2$aTPR, resizable = T)
```

##### 2.2.3 Output3: pOCR power curve (left) and aTPR power curve (right)
```{r echo=F,fig.height=6, fig.width=16}
res2$plot
```
